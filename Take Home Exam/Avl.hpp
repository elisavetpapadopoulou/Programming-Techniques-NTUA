...
private:
  ...
  struct node {
    ...
    node(const T &x, node *p = nullptr)
        : data(x), size(1), balance(EH), left(nullptr), right(nullptr), parent(p) {}
  };
  ...
  void replace_child(node *t, node *old_child, node *new_child) {
    if (t == nullptr) {
      //new_child->size = root->size;
      root = new_child;
    }
    else if (old_child == t->left) {
      //new_child->size = old_child->size;
      t->left = new_child;
    }
    else {
    	//new_child->size = old_child->size;
...
private:
  ...
  node *double_rotate(node *B, node *A, signed char sign) {
    ...
    if (E != nullptr) {
    	B->size = B->size - E->size;
    	E->size = smallRootSize;
    	A->size = A->size - E->size;
    	E->size = RootSize;
    }
    ...
    if (G != nullptr) {
    	G->parent = A;
    	A->size = A->size + G->size;;
    }
    if (F != nullptr) {
    	F->parent = B;
    	B->size = B->size + F->size;
    }
    replace_child(P, A, E);
    return E;
  }
...
private:
  ...
  void remove(node *t) {
  	...
  	if (t->left != nullptr && t->right != nullptr) {
  		...
  		p = swap_with_successor(t, left_deleted); //1
...
public:
  Iterator<T> rank(int n) {
	  ...
	  if (the_size <= n or n < 0) return end();
	  int rank=0;
	  if (t->right != nullptr)
		  rank = t->size - t->right->size - 1; // n of current node
	  else
		  rank = t->size -1;
	  while (true) {
		  if (n < rank) {
